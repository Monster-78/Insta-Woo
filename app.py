#!/usr/bin/env python3
"""
WooCommerce → Instagram Auto-Poster (Railway/Render friendly)
"""
...
def ig_post(url, data):
    ...
    if r.status_code >= 400:
        raise IGError(f"HTTP {r.status_code}: {r.text}")
    return r.json()

def publish_single(r):
    imgs = json.loads(r["images_json"]) if r["images_json"] else []
    if not imgs:
        mark_item(r["id"], "skipped", "no image")
        return
    try:
...
def update_metrics():
    ...
    with _db_lock:
        conn = db()
        conn.execute("INSERT INTO metrics(d) VALUES(?) ON CONFLICT(d) DO NOTHING", (today,))
        conn.execute("UPDATE metrics SET posts_made=COALESCE(posts_made,0)+1, last_post_at=datetime('now') WHERE d=?", (today,))
        conn.commit()
        conn.close()
